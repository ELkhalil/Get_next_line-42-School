...
/*   Updated: 2022/11/05 16:02:16 by aelkhali         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	*container[FD_LIMIT];
	char		*line;

	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	container[fd] = re_n_store(fd, container[fd]);
	if (!container[fd])
		return (NULL);
	line = line_extractor(container[fd]);
	container[fd] = extract_mod(container[fd]);
	return (line);
}
